...
class CozyStatsCog(commands.Cog):
    ...
    async def profile_command(self, interaction: discord.Interaction):
        ...
        rank_info = cozy_gamification.get_user_rank(interaction.user.id)
        ...
        # Create embed
        embed = discord.Embed(
            title=f"🌟 {interaction.user.display_name}'s Cozy Profile",
    ...
    async def top_users_command(self, interaction: discord.Interaction):
        ...
        for i, user_data in enumerate(leaderboard, start=1):
            ...
            hours = int(user_data['listening_time'] // 3600)
            minutes = int((user_data['listening_time'] % 3600) // 60)
...
async def setup(bot):
    await bot.add_cog(CozyStatsCog(bot))