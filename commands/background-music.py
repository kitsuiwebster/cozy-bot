...
# BackgroundMusicView class inherits from View and represents the interactive view for background_music sound selection.
class BackgroundMusicView(View):
    def __init__(self, background_music_sounds, user_id, bot):
        super().__init__(timeout=None)
        self.user_id = user_id
        self.bot = bot
        self.background_music_sounds = background_music_sounds
        self.sound_labels = bot.get_cog("BackgroundMusicCog").sound_labels

        # Create buttons for each background_music sound.
        for i, sound in enumerate(self.background_music_sounds):
    ...
    async def on_button_click(self, interaction):
        ...
        if  interaction.custom_id == "stop":
            ...
            await self.bot.get_cog("BackgroundMusicCog").stop_sound(interaction, guild_id)

        else: 
            # Handle background_music sound button click.
            await self.bot.get_cog("BackgroundMusicCog").on_button_click(interaction)


# BackgroundMusicCog class is a cog for the bot that handles background_music sound related commands.
class BackgroundMusicCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.guild_states = {} 
        # List of background_music sound file names.
        self.background_music_sounds = ["background-music00.mp3", "background-music01.mp3", "background-music02.mp3", "background-music03.mp3", "background-music04.mp3"]
    ...
    # Slash command to play background_music sound.
    @commands.slash_command(name="background-music", description="Play some music in the background.🎶")
    async def _background_music(self, ctx):
        await ctx.defer()
        ...
        # Create and display the BackgroundMusicView with buttons for background_music sound selection.
        view = BackgroundMusicView(self.background_music_sounds, ctx.author.id, self.bot)
        await ctx.respond("Please select a background music:", view=view)
    ...
    async def on_button_click(self, interaction):
        ...
        if interaction.custom_id in self.background_music_sounds:
...
# Function to add the BackgroundMusicCog to the bot.
def setup(bot):
    bot.add_cog(BackgroundMusicCog(bot))
