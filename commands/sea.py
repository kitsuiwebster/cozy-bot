...
# SeaView class inherits from View and represents the interactive view for sea sound selection.
class SeaView(View):
    def __init__(self, sea_sounds, user_id, bot):
        super().__init__(timeout=None)
        self.user_id = user_id
        self.bot = bot
        self.sea_sounds = sea_sounds
        self.sound_labels = bot.get_cog("SeaCog").sound_labels

        # Create buttons for each sea sound.
        for i, sound in enumerate(self.sea_sounds):
    ...
    async def on_button_click(self, interaction):
        ...
        if  interaction.custom_id == "stop":
            ...
            await self.bot.get_cog("SeaCog").stop_sound(interaction, guild_id)

        else: 
            # Handle sea sound button click.
            await self.bot.get_cog("SeaCog").on_button_click(interaction)


# SeaCog class is a cog for the bot that handles sea sound related commands.
class SeaCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.guild_states = {} 
        # List of sea sound file names.
        self.sea_sounds = ["sea00.mp3", "sea01.mp3", "sea02.mp3", "sea03.mp3", "sea04.mp3"]
    ...
    # Slash command to play sea sound.
    @commands.slash_command(name="sea", description="Play the sound of sea.🌊")
    async def _sea(self, ctx):
        await ctx.defer()
        ...
        # Create and display the SeaView with buttons for sea sound selection.
        view = SeaView(self.sea_sounds, ctx.author.id, self.bot)
        await ctx.respond("Please select a sea sound:", view=view)
    ...
    async def on_button_click(self, interaction):
        ...
        if interaction.custom_id in self.sea_sounds:
...
# Function to add the SeaCog to the bot.
def setup(bot):
