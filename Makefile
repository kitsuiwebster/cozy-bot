...
help:
	...
	@echo "⚠️  make stop-all        - Stop ALL services (CouchDB container removed, volume becomes prunable)"
	@echo "⚠️  make rebuild-all     - Rebuild ALL services (recreates CouchDB container)"
	...
	@echo "⚠️  make stop-db          - Stop CouchDB (removes container; volume survives but becomes prunable)"
	@echo "  make restart-db        - Restart CouchDB"
	@echo "⚠️  make rebuild-db       - Recreate CouchDB container (volume should persist if name unchanged)"
	@echo "  make logs-db           - CouchDB logs"
	@echo "⚠️  make seed-db          - Seed CouchDB views (overwrites existing _design docs)"
	@echo ""
	@echo "🧩 Mixed:"
	@echo "  make logs-core        - Bot + Public API logs"
	@echo ""
	@echo "🧹 Maintenance:"
	@echo "🚨 make clean            - Docker system+volume prune (WILL DELETE couchdb-data if no container references it)"
