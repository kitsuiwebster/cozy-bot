...
if [ ! -f ".env" ]; then
    ...
    echo -e "${YELLOW}📝 Create .env in dev/ directory and configure it${NC}"
    exit 1
fi
...
if [ ! -z "$EXISTING_CONTAINER" ]; then
    ...
    if [ $API_EXIT_CODE -eq 0 ] && echo "$API_HEALTH" | grep -q "healthy"; then
        ...
        if [ $? -eq 0 ]; then
            ...
            SESSION_FINALIZE_RESULT=$(curl -k -s -X POST "https://localhost:8001/api/audio/finalize-sessions" 2>/dev/null)
...
STOPPED_CONTAINERS=$(docker compose --env-file .env down --remove-orphans 2>&1 || true)
